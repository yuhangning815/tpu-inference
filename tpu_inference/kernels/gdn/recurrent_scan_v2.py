...
def inner_kernel(
  ...
  """
    ...
    # 2. Decode Branch — restructured to expose final store-inflight state
    # to the outer scope so the drain can happen at the very end of
    # inner_kernel (after prefill), maximally overlapping store DMAs.
    ...
    def decode_branch():
        ...
        # Issue load(0) and load(1) before the fori_loop so they overlap
        # with the loop's first compute. Subsequent loads (iter b+2 for
        # each iter b) are issued from inside the loop as prefetches.
        ...
        @jax.named_scope("process_decode_async_store_double_buffered")
        def process_decode(b, store_inflight):
            ...
            @pl.when(is_valid)
            def do_work():
                # 1. Wait on previous same-slot store from iter b-2 (slot reuse).
                ...
                # 2. Wait for THIS iter's load (issued by preload or by iter b-2).
                ...
                # 3. Copy loaded state to fp32 compute scratch.
                decode_state_scratch[pl.ds(
                    0, 1)] = decode_load_scratch[pl.ds(slot, 1)][...].astype(
                        jnp.float32)
 
                # 4. Prefetch load for iter b+2 (same slot, since (b+2) % 2 == b % 2).
                ...
                # Async store: write to decode_store_scratch[slot] then start
                # DMA from there to HBM. No wait — drained later.
        ...
        return (final_s0_inflight, final_s1_inflight)
 
    def no_decode_branch():
        return (jnp.int32(0), jnp.int32(0))
 
    final_decode_store_inflight = jax.lax.cond(decode_valid > 0,
                                                decode_branch,
                                                no_decode_branch)
    ...
    # ---- Drain async decode store DMAs at the very end of inner_kernel.
    # ---- This is placed AFTER prefill so the in-flight store DMAs overlap
    # ---- with prefill compute. The drain has no data dependency on
    # ---- prefill outputs — only the semaphore handshake.
    final_s0_store_inflight, final_s1_store_inflight = final_decode_store_inflight
 
    @pl.when(final_s0_store_inflight > 0)
    def _drain_decode_store_slot_0():
        dummy = pltpu.make_async_copy(
    ...
    @pl.when(final_s1_store_inflight > 0)
    def _drain_decode_store_slot_1():
        dummy = pltpu.make_async_copy(
